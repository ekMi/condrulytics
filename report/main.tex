...
Le service de parsing traite les fichiers en parallèle avec \texttt{ThreadPoolExecutor}. Il génère également un rapport indiquant le statut de chaque fichier : succès, fichier ignoré, erreur ou différence entre le nombre de participants attendu et le nombre de lignes parsées (lorque cette information est disponible dans le pdf).
...
\begin{enumerate}[label=\arabic*.]
    \item \textbf{Première passe : coureurs fréquents entre eux.} Les coureurs apparaissant au moins trois fois sont comparés. La similarité du nom est calculée avec un score de fuzzy matching à l'aide de la bibliothèque \texttt{RapidFuzz}. Ce score est combiné avec un score de proximité de vitesse. Le score final donne un poids dominant au nom, mais conserve une vérification par la performance. Si deux profils sont suffisamment proches, le nom le plus fréquent devient le nom corrigé. Cette première passe sur les coureurs fréquents permet de créer une base de noms fiables pour la suite du processus.
...
Avec cette convention, un indice de 100 correspond à une performance moyenne dans la course. Un indice de 110 correspond à une performance située un écart-type au-dessus de la moyenne, et un indice de 90 à une performance située un écart-type en dessous. Cette transformation ne change pas l'information, mais elle produit une échelle plus lisible pour les utilisateurs.

Cet indice sera utilisé dans l'application pour montrer l'évolution d'un coureur au fil du temps, indépendamment des distances exactes. Une courte explication est affichée dans l'accueil, dans la vue course et dans la vue coureur afin de rappeler cette lecture au moment où l'utilisateur voit les graphiques et les tableaux.
...
Les variables utilisées sont les suivantes : distance de la course, mois et jour de l'année, nombre de courses précédentes, vitesse moyenne passée, régularité, meilleur niveau observé, performance récente sur trois courses, temps depuis la dernière course et ancienneté du coureur dans l'historique.
...
\subsection{Personas et questions de visualisation}

Pour vérifier que les visuels répondent à des besoins concrets, deux personas simples ont été utilisés comme guide de conception. L'objectif n'est pas de couvrir tous les profils possibles, mais de garder en tête des questions réalistes de coureurs lorsqu'ils ouvrent l'application.

\begin{longtable}{p{3.2cm}p{5.3cm}p{5.4cm}}
\toprule
\textbf{Persona} & \textbf{Besoin principal} & \textbf{Questions auxquelles les visuels doivent répondre} \\
\midrule
\textbf{Claire, coureuse loisir} & Comprendre rapidement ses résultats sans vocabulaire statistique. Elle veut savoir si elle progresse et comment lire l'indice affiché dans l'application. & Est-ce que mes performances s'améliorent au fil du temps ? Sur quelles courses ai-je fait mieux que d'habitude ? Que signifie un indice comme 103 ? \\
\textbf{Marc, coureur régulier} & Comparer ses courses et préparer ses objectifs. Il accepte davantage de détails, mais veut des indicateurs directement actionnables. & Sur quelles distances suis-je le plus performant ? Ai-je fait mieux ou moins bien que le temps attendu ? Quel serait mon niveau sur une course non courue ? \\
\bottomrule
\end{longtable}
